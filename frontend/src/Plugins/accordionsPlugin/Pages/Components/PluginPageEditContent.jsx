...
import { APIGetAccordion, APIUpdateAccordion } from '../../API/APIAccordions';
import { notify } from '@/Utils/Notification';

const PluginPageEditContent = () => {
  ...
  const save = async () => {
    if (title != '') {
      const data = await APIUpdateAccordion(loginPassword, id, title, items);
      if (data.status === 200 && data.data.updateAccordion.post_id) {
        notify('Successfully updated accordion', 'success');
      } else {
        notify('Failed to update the accordion', 'error');
      }
    } else {
      notify('Failed to update the accordion', 'error');
    }
  ...
  return (
    <div className="w-full px-4 py-4">
      <TitleBar title="Accordions Plugin - Edit" save={save} />
...
export default PluginPageEditContent;
