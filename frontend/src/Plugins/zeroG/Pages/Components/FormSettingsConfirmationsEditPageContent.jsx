...
import {
  ...
  APIAllGetPages,
  APIGetFormConfirmation,
  APIGetGForm,
  APIUpdateConfirmationGForm,
  APIUpdateGForm,
} from '../../API/APIForms';
...
const FormSettingsConfirmationsEditPageContent = () => {
  ...
  const saveConfirmation = async () => {
    const data = await APIUpdateConfirmationGForm(
      loginPassword,
      confirmationId,
      id,
      confirmationName,
    ...
    if (data.status == 200) {
      if (data.data.updateGFormConfirmation.success) {
  ...
  const fetchData = async () => {
    ...
    const confirmationData = await APIGetFormConfirmation(confirmationId, id);
    if (confirmationData.status == 200) {
      if (confirmationData.data.getGFormConfirmation != null) {
        const conf = JSON.parse(confirmationData.data.getGFormConfirmation);
        setConfirmationName(conf.name);
        setConfirmationType(conf.type);
        setMessage(conf.message);
        setPage(conf.page);
        setPassData(conf.passData);
        setRedirectUrl(conf.redirectUrl);
      }
    }
  };
...
export default FormSettingsConfirmationsEditPageContent;
