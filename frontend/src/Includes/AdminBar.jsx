...
const Adminbar = ({ page = null, post = null }) => {
  ...
  const handleClearCache = async () => {
    try {
      const data = await APIClearCache(loginPassword);
      if (data?.data?.clearCache?.success) {
        alert(`Cache cleared successfully! ${data.data.clearCache.message}`);
      } else {
        alert('Failed to clear cache: ' + data?.data?.clearCache?.message);
      }
    } catch (err) {
  ...
  return (
    <>
      {(user?.user_role?.role == 'administrator' ||
        ...
        <div className="w-full fixed z-[99999] h-[40px] bg-gray-800 text-white flex justify-between items-center px-4 top-0">
          <div className="flex-center-4">
            ...
            <Link to="/admin" className="flex-center hover:text-gray-400">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth="0"
            ...
            {(page != null || post != null) && (
              <Link
                to={`/admin/${page != null ? 'pages' : 'posts'}/edit/${
                  page != null ? page.id : post.id
                }`}
    ...
    <li>
      <Link to="/admin/" className="header-menu-item-link group">
        <svg
          ...
          width="200px"
          xmlns="http://www.w3.org/2000/svg"
          className="w-4 h-4 header-menu-item-link-icon"
        >
