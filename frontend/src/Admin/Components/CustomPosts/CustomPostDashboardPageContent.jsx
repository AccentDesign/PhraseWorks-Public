...
const CustomPostDashboardPageContent = () => {
  ...
  const fetchData = async (isMounted = true) => {
    ...
    if (isMounted && postTypePost.status == 200) {
      ...
      if (filter == 'all') {
        const data = await APIGetPosts(loginPassword, page, perPage, postType);
        if (isMounted && data.status == 200) {
          setPosts(data.data.getPosts.posts);
          setTotalPosts(data.data.getPosts.total);
        }
      } else if (filter == 'mine') {
        const data = await APIGetPostsByAuthor(page, perPage, postType, user.id);
        if (isMounted && data.status == 200) {
          setPosts(data.data.getPostsByAuthor.posts);
        }
      } else {
        const data = await APIGetPostsByStatus(loginPassword, page, perPage, postType, filter);
        if (isMounted && data.status == 200) {
          setPosts(data.data.getPostsByStatus.posts);
        }
      }
      const allData = await APIAllGetPosts(loginPassword, 1, 2147483647, postType);
      if (isMounted && allData.status == 200) {
        setAllPosts(allData.data.getPosts.posts);
      }

      const fieldsData = await APIGetPostTableFields(loginPassword, postType);
      if (isMounted && fieldsData.status == 200) {
  ...
  return (
    <>
      <div className="w-full">
        <TitleBar postType={postType} customPostName={customPostName} />
        <Filter
          posts={allPosts}
...
export default CustomPostDashboardPageContent;
