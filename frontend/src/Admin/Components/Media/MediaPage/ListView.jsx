...
const ListView = ({
  ...
  useEffect(() => {
    if (reloadFiles) {
      ...
      setCacheBuster(`?t=${Date.now()}`); // force fresh image load
      setReloadFiles(false);
    }
  }, [reloadFiles, files, setReloadFiles]);
              ...
              <td>
                <div className="flex flex-row items-center">
                  {file.mimetype.includes('image') ? (
                    <img
                      src={`http://localhost/uploads/${thumbnail}${cacheBuster}`}
                      className="media-preview-img-cover"
                    ...
                    <svg
                      ...
                      className="media-preview-img w-4 h-4"
                    >
                  ...
                  <div className="ml-4 flex flex-col items-start justify-start">
                    ...
                    <button className="link" onClick={() => ShowDetails(file.id)}>
                      View
                    </button>
                  </div>
                </div>
              </td>
              <td>{file?.author?.user_login || '-'}</td>
              <td>-</td>
              <td>{file?.date ? new Date(file.date).toLocaleDateString() : '-'}</td>
            </tr>
          );
        })}
      </tbody>
