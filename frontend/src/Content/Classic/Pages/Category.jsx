import React, { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';

import Header from '../Components/Header';
import Footer from '../Components/Footer';
import PageContent from '../Components/PageContent.jsx';
import { get_category, get_posts_by_category } from '@/Includes/Functions';
import PostsList from '@/Includes/Shortcodes/PostsList.jsx';
import Pagination from '@/Includes/Shortcodes/Pagination.jsx';
import HeadMeta from '@/Utils/HeadMeta.jsx';
import ContentWrapper from '../../../Includes/ContentWrapper.jsx';
import Adminbar from '../../../Includes/AdminBar.jsx';

const Author = () => {
  const { categoryName } = useParams();
  const [category, setCategory] = useState(null);
  const [posts, setPosts] = useState([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(8);

  useEffect(() => {
    const fetchData = async () => {
      const data = await get_category(categoryName);
      setCategory(data);
      const postsTmp = await get_posts_by_category(1, 20, data.term_id);
      setPosts(postsTmp.data);
      setTotal(postsTmp.total);
    };
    fetchData();
  }, [page, categoryName]);

  return (
    <>
      <HeadMeta
        pageTitle="Category"
        description="Browse posts and content grouped under this category to easily find related topics."
      />
      <div className="bg-gray-200 content min-h-screen top-0 flex flex-col">
        <ContentWrapper>
          <Adminbar />
          <Header />
          <PageContent>
            <h2 className="text-4xl font-bold mb-4">Category: {category?.name}</h2>
            <PostsList posts={posts} />
            <Pagination total={total} page={page} perPage={perPage} setPage={setPage} />
          </PageContent>
        </ContentWrapper>
        <Footer />
      </div>
    </>
  );
};

export default Author;
